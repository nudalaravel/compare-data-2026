import { _ as _sfc_main$8 } from "./LegacyPanel-DJJ9u9fC.js";
import { computed, unref, withCtx, createVNode, openBlock, createBlock, Fragment, renderList, toDisplayString, createCommentVNode, useSSRContext, mergeProps, useModel, ref, watch, withKeys, withModifiers, createTextVNode, mergeModels, isRef } from "vue";
import { ssrRenderAttrs, ssrInterpolate, ssrRenderClass, ssrRenderComponent, ssrRenderList, ssrIncludeBooleanAttr, ssrRenderAttr, ssrRenderStyle, ssrRenderTeleport, ssrRenderSlot } from "vue/server-renderer";
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ssrRenderTeleport(_push, (_push2) => {
        if (__props.open) {
          _push2(`<div class="modal-backdrop"><section class="confirm-dialog" role="dialog" aria-modal="true"${ssrRenderAttr("aria-label", __props.title)}><header><h2>${ssrInterpolate(__props.title)}</h2><button type="button" aria-label="ปิด">×</button></header><div class="confirm-content">`);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(isAuthenticated)) {
        ...
        _push(ssrRenderComponent(_component_CommonConfirmDialog, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`<p${_scopeId}>ระบบจะตรวจตาราง <strong${_scopeId}>${ssrInterpolate(unref(preview)?.targetFullName)}</strong> และคัดลอกเฉพาะข้อมูล <strong${_scopeId}>round = 1</strong> จากฐานต้นทางไปยังฐาน <strong${_scopeId}>_cmp</strong></p><ul class="confirm-list"${_scopeId}><li${_scopeId}>ไม่แก้ไขหรือลบข้อมูลในฐานต้นทาง \`_raw\`</li><li${_scopeId}>ใช้ Transaction และ Rollback เมื่อเกิดข้อผิดพลาดใน PHP API จริง</li><li${_scopeId}>บันทึกผลและ Audit Log เฉพาะเมื่อผู้ใช้กด Submit</li></ul>`);
            } else {
              return [
                createVNode("p", null, [
                  createTextVNode("ระบบจะตรวจตาราง "),
                  createVNode("strong", null, toDisplayString(unref(preview)?.targetFullName), 1),
...
//# sourceMappingURL=index-DUI1LL9G.js.map
