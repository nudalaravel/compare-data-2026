        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ssrRenderList(unref(visibleRows), (row) => {
              ...
              ssrRenderList(unref(tableColumns), (table) => {
                _push2(`<td data-v-63040254${_scopeId}><span class="${ssrRenderClass(["legend ", row.tables?.[table.tableName]?.tone || "empty"])}"${ssrRenderAttr("title", row.tables?.[table.tableName]?.label || "")} data-v-63040254${_scopeId}>${ssrInterpolate(row.tables?.[table.tableName]?.icon || "--")}</span></td>`);
              });
              _push2(`<!--]--></tr>`);
            });
            _push2(`<!--]-->`);
...
//# sourceMappingURL=report-DsXlrhCs.js.map
